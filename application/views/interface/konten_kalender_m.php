...
?>
		...
		<table class="table table-bordered">
				...
				endforeach;
					...
					SELECT day(tgl_cekin) cin from tbl_transaksi
					WHERE MONTH(tgl_cekin) = '{$mon}' and YEAR(tgl_cekin) = '{$tahun}' AND kode_villa = '{$row->kode_villa}'
					")->result();
					$qcout_sign = $this->app_model->manualQuery("
					SELECT day(tgl_cekout) cout from tbl_transaksi
					WHERE MONTH(tgl_cekout) = '{$mon}' and YEAR(tgl_cekin) = '{$tahun}' AND kode_villa = '{$row->kode_villa}'
					")->result();
							$object = new stdClass();
							...
							// print_r($object);
							foreach($qcin_sign as $cin) :
								$cekin = str_pad($cin->cin,2,"0",STR_PAD_LEFT);
								$tcekin = "i_".$cekin;
								$object->$tcekin = "in";
							endforeach;
							foreach ($qcout_sign as $cout) :
								$cekout = str_pad($cout->cout,2,"0",STR_PAD_LEFT);
								$tcekout = "o_".$cekout;
								$object->$tcekout = "out";
							endforeach;
						...
						$ovar = "o_".$apad;
							...
							?><td class="<?=$class?>"><?php 
								...
								if (EMPTY($object->$var)) :
									...
									if (EMPTY($object->$ivar)) :
									else:
										echo @$object->$ivar;
									endif;
									if (EMPTY($object->$ovar)) :
									else:
										echo @$object->$ovar;
									endif;
