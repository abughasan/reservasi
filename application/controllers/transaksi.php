...
class Transaksi extends CI_Controller {
	...
	function view(){
		...
		$data['detail_transaksi'] = $this->db->query("select * from tbl_transaksi 
		where id_status_v = 2
		order by tgl_cekin, tgl_transaksi ASC");
		...
		//QUERY DETAIL
		$data['salahquery'] = $this->app_model->manualQuery
			("SELECT
				tr.*, tm.*, vl.*, byr.*
			FROM
				tbl_transaksi tr
			...
			left JOIN 
				tbl_pembayaran byr
				ON tr.no_transaksi = byr.no_transaksi	WHERE tr.id_status_v = 2		
			");	
	...
	function view_where($nama,$opt){
		// IF ($nama==0){$nama="";}
		if ($nama == '0' or $nama == '%20') {$nama = '';}
		// $data['status_v'] = '2';
		// $this->session->set_userdata($data);
		//definisikan nama table dulu biar mudah copas bikin controller
		$table = 'tbl_transaksi';
		...
		tbl_tamu tm
		ON tr.id_tamu = tm.id_tamu
		WHERE tm.nama_tamu LIKE '%{$nama}%' AND tr.id_status_v = {$opt}
		...
		$data['template'] = 'satucolumn';
		if ($opt=='1') {
			// $data['interface'] = array('data_transaksi_cancel');		
		...
		if ($opt=='3') {
			$this->load->view('interface/konten_data_transaksi_checkin',$data);
		}
		if ($opt=='4') {
			$this->load->view('interface/konten_data_transaksi_checkout',$data);
		}
	}
	...
	function updatecanceltrans($hadirval,$no_transaksi)
	{
		...
		");		
		$this->app_model->manualQuery("
		DELETE FROM transaksi WHERE bridge_transaksi = '".$no_transaksi."'
		");
	}
